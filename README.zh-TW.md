...
Sales AI 的長期目標是**把一家公司的銷售職能徹底自動化**——讓 AI 像一位永不下班的資深客戶經理那樣工作：24 小時讀客戶來信、看懂商業脈絡、評估風險、提出對雙方都划算的方案，最後只把真正需要人簽字的少數例外交回手上。

今天我們交付的是一個以脈絡為本、有硬性主管核准門檻的 MVP。未來這道門檻仍然存在——但只在真正敏感的情境（退款、合約讓步、VIP 客戶流失訊號）才觸發；其餘日常往返的八成，由 agent 從頭到尾自主完成。下方的 roadmap 就是從現在走到那裡的路徑。
