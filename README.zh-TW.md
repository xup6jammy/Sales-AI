...
Sales AI 的目標是做出一款 **24 小時不下班、專注於業務職能的自動化 AI agent**——一台不眠不休的銷售引擎。

它不限定產業。**只要接上你們既有的客戶資料庫、會議紀錄、合約檔案與 CRM 系統**，這套 agent 就能像一位資深業務一樣 24 小時依照郵件脈絡回應客戶——白天的詢價、半夜的退款訴求、週末的續約問題，全部第一時間接住。從製造業、金融業、SaaS、跨境電商到 B2B 服務業，**只要你的客戶會寫信來，這套 agent 就能落地**。

### 未來路線

- **Phase 4 — 主動式外呼。** 不只是被動回信，而是主動觸達。針對逾期的提案、快到期的合約、明顯的流失訊號，agent 自己擬稿、自己排優先順序、自己把跟進時程排好。
- **Phase 5 — 跨通路留言。** 從 email 擴展到 LinkedIn、WhatsApp、LINE OA、Slack、官網對話框、社群媒體 DM——同一份客戶脈絡，跨多個通路的一致回應。
- **Phase 6 — 自主成交。** 在主管預先定義的價格區間、合約範本、折扣權限內，agent 可以自主完成從報價、議約、合約簽署到 CRM 結案的整段流程，把日常成交真正搬離人手。

### 這對公司值多少

- **業績不再卡在「業務人手不夠」。** 5000 個客戶的公司原本要 30 個業務才照顧得到；接上這套 agent 之後，5 人 + AI 也能跑出更高的客戶接觸密度。
- **回應速度變成競爭武器。** 業界平均詢價回信要 4-12 小時，agent 30 秒內回——**速度本身就是成交率**。
- **公司知識永遠不會走人。** 業務離職把客戶記憶、talking point、過往交易脈絡帶走，是 B2B 公司最痛的事。Sales AI 把這份知識常駐在資料庫裡，agent 隨時上工就帶著走。
- **主管把時間花在該花的地方。** 退款、讓步、VIP 風險這些例外才需要簽字；其餘八成的日常往返，不再吃時間。
- **可稽核 = 法遵級的可信任。** 受監管產業（銀行、保險、醫療）的 agent 不能是黑盒子。Sales AI 每一步都寫進稽核紀錄，金管會、董事會、外部審計都能查到這封郵件**為什麼**這樣回。

今天交付的是一個以脈絡為本、有硬性主管核准門檻的 MVP。隨著 phase 推進，門檻越收越窄、自主跑的部分越來越大——但**門檻永遠不會消失**。這是我們的安全承諾。

## 為什麼選 Java

不是 Java 潮，而是這專案要去的地方就是 Java 的主場。

| 理由 | 在這個專案裡怎麼展現 |
|---|---|
| **B2B 企業就是 Java 的地盤** | 銀行、保險、製造、ERP/CRM 後端九成是 Java/Spring。這個 agent 要嵌進去，跟既有 service 同進程跑、共用稽核 log、共用 DI 容器，比 Python sidecar 阻力小很多。 |
| **JDK 21 + 零依賴 = 60 秒可重現** | 沒有 `pip install` 解依賴衝突、沒有 venv、沒有 `node_modules` 黑洞。`git clone && javac && java` 三步到位，是 Python/Node 做不到的乾淨。 |
| **records + sealed types 寫領域模型** | 13 個 record 拿掉樣板碼後，比 Python `@dataclass` 更安全——編譯期檢查 null、enum 窮舉、switch expression 強制覆蓋全部 case。 |
| **可稽核性是法遵級需求** | 退款／法務／合約這種紅線，編譯期就能保證 [`RiskRules.java`](src/main/java/com/example/salesai/risk/RiskRules.java) 走完所有 enum 分支；Python 的 `if/elif` 漏一個 case 會靜默通過，Java 編譯器會擋下來。 |
| **與 MCP 生態互補** | MCP server 大多是 TS/Python，但工具層不限定。`SKILL.md` 才是 agent 本體，引擎用什麼語言不重要——選 Java 是策略性卡位「企業內部系統好嵌入」這塊。 |

GitHub 上「Java 寫 AI agent」是稀缺品（Python 範例佔 95% 以上）。對企業 IT 部門來說，能直接放進現有 Java 後端的 AI agent，反而是更有實戰價值的差異化武器。
...
如果你想參與把 Sales AI 從一個 GitHub demo 推到真正的產品——而不只是按 star 然後就走——歡迎聯繫。

我在找：

- **B2B 業務 / 客戶經理**：受夠了 email + CRM + 主管核准的日常折磨，願意拿真實工作流程試用副駕駛的人。
- **Java 工程師**：對「在企業環境（銀行、保險、製造、ERP——這些 Java 是母語的地方）落地 AI agent」有興趣的人。
- **投資人 / 共同創辦夥伴**：相信銷售自動化是下一波，願意支持「Java first、稽核優先」這條路線的人。
- **Design partner**：願意把 agent 拉進自家業務團隊試跑、用真實客戶資料一起形塑路線圖的公司。

📩 **a0925281767s@gmail.com**

告訴我你屬於哪一類、你的客戶長什麼樣、什麼樣的功能對你最有價值。從「有趣的 demo」走到「能出貨的產品」，最快的路徑就是跟未來真正會用的人一起把它做出來。
