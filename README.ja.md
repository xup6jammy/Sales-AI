...
Sales AI の長期目標は、**企業のセールス機能を完全に自動化すること**です&mdash;&mdash;AI を、決して退勤しないシニアアカウントマネージャーのように働かせる：24 時間体制で顧客メールを読み、商業的な背景を理解し、リスクを評価し、双方にとって有利な提案を出し、本当に人間のサインが必要な少数の例外だけをエスカレーションするコパイロットです。

今日出荷しているのは、コンテキストファーストでハードなマネージャー承認ゲートを持つ MVP です。将来もこのゲートは残りますが、本当にセンシティブなケース（返金、契約上の譲歩、VIP の解約サイン）でのみ作動し、残りの 8 割の日常的なやり取りはエージェントがエンドツーエンドで自律的に処理します。下の roadmap は、今からそこへ向かう道筋です。
