...
Sales AI의 장기 목표는 **기업의 영업 기능을 완전히 자동화하는 것**입니다&mdash;&mdash;AI를 결코 퇴근하지 않는 시니어 어카운트 매니저처럼 일하게 만드는 것: 24시간 고객 이메일을 읽고, 비즈니스 컨텍스트를 이해하며, 리스크를 평가하고, 양측에 모두 유리한 거래를 제안하며, 정말로 사람의 서명이 필요한 소수의 예외만을 에스컬레이션하는 코파일럿.

오늘 출시되는 것은 컨텍스트 우선이며 하드한 매니저 승인 게이트가 있는 MVP입니다. 미래에도 이 게이트는 남아 있지만, 진정으로 민감한 케이스(환불, 계약상 양보, VIP 이탈 신호)에서만 작동하고, 나머지 80%의 일상적인 왕래는 에이전트가 처음부터 끝까지 자율적으로 처리합니다. 아래의 roadmap이 지금부터 그곳으로 가는 경로입니다.
